...
def plot_history(history, fold_no):
    save_location= 'C:/Users/trg14/Documents/Files học tập/Files học tập/Nghiên cứu Raman trong việc phát hiện bệnh tiểu đường/Training Datas in 16-01-2024/truong_3'
    plt.figure()
...
raw_data = pd.read_csv('16-1-2024/2_datas_labels_truong/datas.csv', header=None)
raw_data = raw_data.to_numpy()
raw_labels = pd.read_csv('16-1-2024/2_datas_labels_truong/labels.csv')['has_DM2']
raw_labels = np.array([int(x) for x in raw_labels])

# data = raw_data
# target = raw_labels
# print(raw_labels)
# raw_data = Cf.Load_compile_data(data_folder, data_file_name)
# raw_data = np.array(raw_data)
# raw_labels = Cf.read_csv_file(label_folder + '/' + label_file_name)
# raw_labels = np.array([int(x) for x in raw_labels])

data, target, new_labels_order = Cf.shuffle_data(raw_data, raw_labels)
data = np.array(data)
...
for i in range(fold):
    ...
    early_stopping = EarlyStopping(monitor='val_loss', patience=50, verbose=0, mode='auto', restore_best_weights=True)
    batch_size = 8
    no_epochs = 200
    ...
    model.add(Conv1D(filters=400, kernel_size=12, activation='relu', name="Conv1D_1", kernel_regularizer=l2(0.05)))
    model.add(MaxPooling1D(pool_size=2))
    model.add(Conv1D(filters=100, kernel_size=5, activation='relu', name="Conv1D_4", kernel_regularizer=l2(0.05)))
    model.add(MaxPooling1D(pool_size=2))
    model.add(Conv1D(filters=50, kernel_size=3, activation='relu', name="Conv1D_5", kernel_regularizer=l2(0.05)))
    model.add(MaxPooling1D(pool_size=2))
    model.add(Flatten())
